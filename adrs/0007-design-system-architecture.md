...
- Dropdown만 `@base-ui/react`의 Select로 만든다. 네이티브 `select`는 목록 스타일링이 불가능하다. Base UI가 요구하는 `isolation: isolate`를 앱 루트에 둔다.
...
- 텍스트는 `--text-12`~`--text-18`과 행간 토큰에 Tailwind `font-*`를 조합한다(`Pr_16_SB600` → `text-16 font-semibold`). 라운드는 Figma 값(4, 6, 8, 10, 12, 14px)만 `--radius-*`로 두고, 간격은 Tailwind 기본 척도를 쓴다.
