...
- 커밋과 푸시는 매번 확인을 받은 뒤에만 실행한다. 메시지와 파일 목록을 먼저 보여준다. "시작하자", "진행해"는 구현 승인이며 커밋 승인이 아니다. 체크포인트는 착수(작업 목록 확인), 커밋 전(코드 검토), 합병 세 곳이다. `.claude/hooks/guard.js`가 커밋, 푸시, PR 생성·합병·닫기, `git reset`, 브랜치 삭제를 승인 토큰 없이는 차단한다. 체크포인트에서는 `AskUserQuestion`(header `🔒 체크포인트`, 선택지 `승인`/`수정 필요`, 첫 줄에 체크포인트 종류와 승인 시 실행되는 명령)을 띄우고, 사용자가 `승인`을 고르면 `.claude/hooks/approve.js`가 토큰을 만들어 커밋, 푸시, PR 생성까지 통과하며 PR 생성에서 토큰이 닫힌다. 채팅에 `승인`이라고 답해도 같다. 합병은 별도 승인이다. 토큰은 30분 뒤 무효가 되며 에이전트는 토큰을 만들지 않는다.
- 화면 단위 기능은 `/feature`, 공용 컴포넌트는 `/component` 스킬로 시작한다. 스킬의 체크포인트에서는 보고만 하고 답을 기다린다.
...
- `feat` 이슈는 착수 전에 `specs/TEMPLATE.md` 형식으로 `specs/<이슈번호>-<이름>.md`를 쓴다. 규칙(`R-xx`)과 흐름(`S-xx`)의 ID를 테스트 이름에 그대로 쓰고, `[확인 필요]`가 남아 있으면 구현을 시작하지 않는다. `shared/ui` 컴포넌트는 스펙 대신 이슈 본문의 변형 표를 쓴다(ADR-0009).
...
- `app` 레이어의 폴더 이름은 내용의 종류(`providers`, `hooks`)가 아니라 목적을 가리켜야 한다. steiger의 `segments-by-purpose`가 강제한다. 라우트는 `app/routes/`에 두고 프로바이더는 루트 파일(`app-providers.tsx`)로 둔다.
