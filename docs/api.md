...
HTTP 클라이언트의 동작과 API 호출 코드의 배치를 정한다. API를 호출하는 코드를 쓰거나 고칠 때 읽는다. 라이브러리, 갱신 전략, 오류 규격의 근거는 ADR-0006에 있고, 오류 문구 규칙은 ADR-0006을 대체한 ADR-0012에 있다.
...
| `token-refresher`     | 갱신 단일 실행, 진행 중 Promise 조회, 갱신 한 번당 `onUnauthorized` 1회 알림 | 동시 호출 1회 실행, 실패와 예외 처리, 알림 초기화   |
...
| `http-client-context` | 클라이언트 인스턴스를 React 트리에 넣는 프로바이더와 `useHttpClient()` 훅    | 프로바이더가 넘긴 인스턴스 반환, 프로바이더 밖 호출 |
...
세션이 바뀌면 쿼리 캐시를 비운다. `initializeSystem()`이 세션 아톰을 구독해 로그인, 회원가입, 갱신 실패 어느 경로로 바뀌든 한 곳에서 처리한다. 캐시가 남으면 같은 브라우저에서 계정을 바꿨을 때 이전 사용자의 목록이 보인다.

`auth` 옵션 없이 만든 클라이언트는 정상 구성이다(ADR-0020). 헤더를 붙이지 않고 만료 확인도 하지 않으며, 401을 `unauthorized` `ApiError`로 던진다. 토큰 갱신 요청은 이 구성의 전용 클라이언트로 보낸다.
...
- 갱신이 실패하면 대기 중이던 모든 요청은 `unauthorized` `ApiError`로 실패하고 `onUnauthorized()`는 그 갱신에 대해 정확히 한 번만 호출된다. 중복 방지는 갱신 한 번의 범위이며 새 갱신을 시작할 때 초기화된다. 로그아웃 뒤 다시 로그인한 세션에서 갱신이 실패해도 알림이 삼켜지지 않아야 하기 때문이다.
